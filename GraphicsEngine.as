package Evocati
{
	...
	{
		public static var _instance:GraphicsEngine;
		...
		/**设置批量纹理的大小
		 */
		public function setBatchTextureSize(value:Number):void
		{
			commonData.batchTextureSize = value;
		}
		...
		{
			textureManager.getTextureFromBitmapData(bitmapData,id,size);
		}
		/**
		 * 从字节流数据得到压缩的纹理
		 */
		...
		{
			textureManager.getCompressedTextureFromByteArray(data,id,size,alpha);
		}
		/**
		 * 从位图数据得到地图纹理
		 */
		...
		{
			textureManager.getMapTextureFromBitmap(bitmap,id,size);
		}
		...
		protected function render(event:Event):void
		{
			...
			context3D.setProgram ( shaderManager.getShaders('MAP','COMMON'));
